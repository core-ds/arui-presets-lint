...
При установке или обновлении `arui-presets-lint` автоматически создаёт (или обновляет) файл `AGENTS.md` в корне вашего проекта. В него добавляется краткий блок с ключевыми правилами линтеров на английском языке, чтобы AI-ассистенты (Cursor, Copilot, Claude Code и т.п.), которые читают `AGENTS.md`, сразу писали код по принятым в компании стандартам и не нарушали правила ESLint / Stylelint / Prettier.

Блок ограничен маркерами и обновляется автоматически при обновлении версии пакета:

```md
<!-- BEGIN arui-presets-lint -->
... автогенерируемое содержимое ...
<!-- END arui-presets-lint -->
```

Ваш собственный текст в `AGENTS.md` вне этих маркеров сохраняется без изменений. Файл перезаписывается только тогда, когда содержимое действительно поменялось, поэтому лишних изменений в git не возникает.

### Ручной запуск

Если проект устанавливается с `--ignore-scripts` (postinstall-хук не выполняется) либо нужно обновить блок вручную, выполните:

```sh
yarn arui-presets-lint agents
```

Команда синхронизирует `AGENTS.md` в текущей директории и выводит путь к файлу и признак того, был ли он изменён.

### Отключение

Чтобы отключить автоматическую синхронизацию `AGENTS.md` на этапе установки, задайте переменную окружения `ARUI_PRESETS_LINT_SKIP_AGENTS` (с любым непустым значением):

```sh
ARUI_PRESETS_LINT_SKIP_AGENTS=1 yarn install
```
